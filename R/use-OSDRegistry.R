...
saveRDS(res, file='cached-copy-OSDRegistry.rds')
...
d <- do.call('rbind', z)
# save
write.csv(d, file=gzfile('parsed-data-OSDRegistry.csv.gz'), row.names=FALSE)
...
d <- do.call('rbind', z)
# save
write.csv(d, file=gzfile('parsed-site-data-OSDRegistry.csv.gz'), row.names=FALSE)
